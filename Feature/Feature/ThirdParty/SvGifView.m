...
+ (void)startGifAddedToView:(UIView *)view
{
    NSURL *fileUrl = [[NSBundle mainBundle] URLForResource:@"common_loading_gif" withExtension:@"gif"];
    SvGifView *gifView = [[SvGifView alloc] initWithCenter:CGPointMake(SCREEN_WIDTH/2, SCREEN_HEIGHT/2) fileURL:fileUrl];
...
+ (BOOL)stopGifForView:(UIView *)view
{
    SvGifView *gifView = [SvGifView gifForView:view];
    if (gifView != nil) {
...
+ (id)gifForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (SvGifView *)subview;
        }
    }
    return nil;
}
